...
* Servlet是JavaEE规范之一，是一个接口，我们需要定义Servlet类实现Servlet接口，并由web服务器运行Servlet
...
![Servlet执行流程](./assets/Servlet执行流程.png)
...
介绍完Servlet的执行流程后，我们知道Servlet是由Tomcat Web服务器帮我们创建的。Tomcat什么时候创建的Servlet对象?下面我们对Servlet的生命周期进行一个介绍。
...
![Servlet体系结构](assets/Servlet体系结构.png)

- B/S架构的web项目，都是针对HTTP协议，所以我们自定义Servlet,会通过继承HttpServlet
...
- 解答：
  ...
  - 前端发送GET和POST请求的时候，参数的位置不一致，GET请求参数在请求行中，POST请求参数在请求体中，为了能处理不同的请求方式，我们得在service方法中进行判断，然后写不同的业务处理，这样能实现，但是每个Servlet类中都将有相似的代码，针对这个问题， 我们可以对Servlet接口进行继承封装，来简化代码开发。
...
类似MyHttpServlet这样的类Servlet中已经为我们提供好了，HttpServlet做的事更多，不仅可以处理GET和POST还可以处理其他五种请求方式。
...
- 解答: 能、能、demo8，进而我们可以得到的结论是`/user/*`中的`/*`代表的是零或多个层级访问目录同时精确匹配优先级要高于目录匹配。
...
:::info 小结
    ...
    2. 五种配置的优先级为 精确匹配 > 目录匹配> 扩展名匹配 > /* > / ,无需记。
